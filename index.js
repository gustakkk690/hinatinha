...
const { help } = require('./FADHIL/listmenu')
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN: BRUXINHO MODS\n'//GANTI NAMA LU COK
            + 'ORG:CRIADOR GUSTA;\n'//GANTI NAMA LU!!
            + 'TEL;type=CELL;type=VOICE;waid=556295620702:+55 62 9562-0702\n'//GANTI NOMOR LU
            + 'END:VCARD'
...
async function starts() {
	...
	client.on('open', () => {
		success('2', 'Escaneie o QRcode acima!')
	})
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const FadhilGraphy = ["556695620702@s.whatsapp.net"]
			...
			const ownerNumber = [
			"556295620702@s.whatsapp.net"//GANTI NOMOR LU
			]
			premium = [
			"556295620702@s.whatsapp.net","556193845817@s.whatsapp.net"//GANTI NOMOR YAG MAU DI PREM, GABISA NYIMPEN DI DATABASE!!
			]
...
/*********END PISAH MENU***********/
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break
				...
				case 'owner':
				case 'creator':
                 client.sendMessage(from, {displayname: "+55 62 9562-0702", vcard: vcard}, MessageType.contact, { quoted: mek})
         ...
         case 'lirik':
                ...
                case 'report':
                    ...
                    reply('Problemas foram relatados ao proprietário , relatórios falsos não serão respondidos..')
                    break
              case 'request':
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
                    ...
                    client.sendMessage('+55956295620702@s.whatsapp.net', options, text, {quoted: mek})
                    reply('As solicitações foram relatadas ao Proprietário , as solicitações que podem sobrecarregar o proprietário não serão respondidas.')
                    break
				case 'meme':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
	   ...
	   case 'coli':
		    ...
		    case 'map':
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 HINATINHA BOT〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case 'clearall':
					...
					reply(`\`\`\`deleta todo o chat do Bot\`\`\``)
					break
                                case 'bcgc':
				...
				default:
                  ...
                  reply(`Desculpe man *${pushname2}*, Comando *${prefix}${command}* comando nn registrado no menu! pfv chame o bot novamente *${prefix}menu!*`)
					...
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[Hinatinhabot]','Yellow'), 'Command Tidak Terdaftar', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'yellow'))
